  ...
  if (!product) {
    return (
      <div className="mx-auto max-w-6xl px-4 py-12">
        <h1 className="font-display text-4xl text-ink-900">Urun bulunamadi</h1>
        <p className="mt-3 text-sm text-ink-500">Bu urun mevcut degil veya kaldirildi.</p>
        <Button to="/products" className="mt-6">Urunlere geri don</Button>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-6xl px-4 py-12">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            ...
            <p className="text-sm text-ink-500">({product.numReviews} yorum)</p>
          </div>
        </div>
        <div className="rounded-3xl border border-sand-200 bg-white p-5 shadow-soft w-full md:w-auto md:min-w-[320px]">
          ...
          <p className="mt-2 text-3xl font-semibold text-ink-900">{formatPrice(product.price)}</p>

          <div className="mt-4 flex items-center justify-between gap-3">
            <label className="text-sm text-ink-600" htmlFor="qty">Almak istenilen adet</label>
            <input
              id="qty"
              type="number"
              min={1}
      ...
      <div className="mt-10 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-3xl border border-sand-200 bg-white p-6">
          <p className="text-xs uppercase tracking-widest text-ink-400">Urun bilgisi</p>
